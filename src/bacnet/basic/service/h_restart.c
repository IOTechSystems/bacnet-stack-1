...
static void Send_Restart_Remote(
    BACNET_ADDRESS *dest, BACNET_TIMESTAMP *tm, BACNET_RESTART_REASON reason)
{
    ...
    bacapp_property_value_list_init (&props[0], 3);
    ...
    props[1].value.tag = BACNET_APPLICATION_TAG_TIMESTAMP;
    bacapp_timestamp_copy (&props[1].value.type.TimeStamp, tm);
...
void handler_restart_send(BACNET_RESTART_REASON reason)
{
    BACNET_DATE_TIME now;
    BACNET_TIMESTAMP tm;

    Device_getCurrentDateTime (&now);
    bacapp_timestamp_datetime_set (&tm, &now);

    for (unsigned index = 0; index < MAX_RESTART_RECIPIENTS; index++) {
        if (Restart_Notification_Recipients[index].tag == 1) {
            Send_Restart_Remote(&Restart_Notification_Recipients[index].type.address, &tm, reason);
        }
    }
}
...
void handler_restart_init(void)
{
    ...
    /* default value is a single broadcast element */
    Restart_Notification_Recipients[0].tag = 1;
    Restart_Notification_Recipients[0].type.address.net = BACNET_BROADCAST_NETWORK;
}
